function [P_total, P_max, W, P_each] = Precoder_WB( SP, G, n, lambda)
...
for k = 1:Nsc
    ...
    %         ICI = H(:,:,i)*diag(lambda(:))*H(:,:,i)';
            ...
            F(:, u , i, k) = pinv(alpha^2*ICI ...
                                + alpha^2*eye(Nr) ...
                                + alpha*(1-alpha)*Psi_k*(diag(diag(Psi'*G_i*LAMBDA*G_i'*Psi))+ kron(eye(Nsc), eye(Nr)))*Psi_k')*G_incell; % incell MMSE
        end
...
%  
    ...
    for i = 1:Nc
        P=0;
        for k = 1:Nsc
            P = (alpha/Nsc)*W(:,:,i,k)*W(:,:,i,k)';
            
            P_each = [P_each; diag(abs(P))];
        end
